...
{
	/// <summary>
	/// CJR形式データを格納　＊＊＊＊＊＊　MultiCJR 対応版　＊＊＊＊＊＊＊
	/// </summary>
	public class CjrFormat2
	{
        private static readonly int INITIAL_LIST_LEN = 32768;

        public enum BaudRate { Low = 0x64, High = 0 };
        public enum WriteType { CJR, BIN };
		...
		{
            List<byte> list = new List<byte>(INITIAL_LIST_LEN);
			int blockSize = 0;
			int currentPoint = 0;
		...
		public bool AddBinData(byte[] binData, string jrFileName, int sAddress, bool basic, bool append = false)
		{
            ...
            {
                blockCount = 0;
                cjrData = new List<byte>(INITIAL_LIST_LEN);
                isClosed = false;
                ...
                BArrayUtil.AddBytesToList(cjrData, bytes); // ファイル名
            ...
            while (true)
			{
                ...
                BArrayUtil.AddBytesToList(cjrData, binArray);
        ...
        public void CloseCjrData()
        {
            ...
            BArrayUtil.AddBytesToList(cjrData, barray);
        ...
        /// <summary>
        /// ボーレート指定を0,または0x64に変更
        /// </summary>
        public void AdjustBaudRate(byte val)
        {
            cjrData[23] = val;
            byte sum = 0;
            for (int i = 0; i < 32; ++i) sum += cjrData[i];
        ...
        public void WriteCjrFile(string outputFilename, WriteType type)
        {
            using (FileStream fs = new FileStream(outputFilename, FileMode.Create, FileAccess.Write))
            using (BinaryWriter writer = new BinaryWriter(fs))
            {
                if (type == WriteType.CJR) //CJR format
                {
                    AdjustBaudRate(0);
                    writer.Write(cjrData.ToArray());
                }
                else // BIN format
                {
                    writer.Write(GetBinData());
                }
            }
        }
